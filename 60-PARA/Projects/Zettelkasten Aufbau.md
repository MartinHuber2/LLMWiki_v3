...
- [ ] **NotebookLM-Übernahme:** Inhalte aus NotebookLM in den Vault übernehmen — Behandlung als `source-type: ai-chat`. NotebookLM erzeugt die Inhalte so aufbereitet, dass sie problemlos ingestiert werden können (Output-Struktur auf `10-Raw/Waiting_For_Ingestion/` abgestimmt). Gibt NotebookLM im Output an, auf welche Originaldateien es verweist bzw. aus welchen es seine Informationen bezieht, übernimmt die KI diese Verweise in den Notiztext — als **Fußnoten**, nicht als Q-Referenzen.
